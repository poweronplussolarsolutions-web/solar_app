...
# Uses the supplied 9-page quotation PDF as the visual template.
# ─────────────────────────────────────────────────────────────────────────────

QUOTATION_ROOT = os.path.join(app.root_path, 'quotation_data')
QUOTATION_TEMPLATE_DIR = os.path.join(QUOTATION_ROOT, 'templates')
QUOTATION_OUTPUT_DIR = os.path.join(QUOTATION_ROOT, 'generated')
os.makedirs(QUOTATION_TEMPLATE_DIR, exist_ok=True)
os.makedirs(QUOTATION_OUTPUT_DIR, exist_ok=True)
QUOTATION_TEMPLATE_PATH = os.path.join(QUOTATION_TEMPLATE_DIR, 'quotation_template.pdf')

class QuotationConfiguration(db.Model):
    __tablename__ = 'quotation_configurations'
    id = db.Column(db.Integer, primary_key=True)
    system_type = db.Column(db.String(50), nullable=False, default='On Grid')
    capacity_kw = db.Column(db.Numeric(10, 2), nullable=False)
    name = db.Column(db.String(255), nullable=False)
    active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    items = db.relationship('QuotationConfigurationItem', backref='configuration', cascade='all, delete-orphan', lazy=True,
                            order_by='QuotationConfigurationItem.sort_order')

class QuotationConfigurationItem(db.Model):
    __tablename__ = 'quotation_configuration_items'
    id = db.Column(db.Integer, primary_key=True)
    configuration_id = db.Column(db.Integer, db.ForeignKey('quotation_configurations.id'), nullable=False)
    ...
    qty_value = db.Column(db.Numeric(10, 2), default=1)
...
class Quotation(db.Model):
    ...
    system_type = db.Column(db.String(50), default='On Grid')
    capacity_kw = db.Column(db.Numeric(10, 2), nullable=False)
    panel_technology = db.Column(db.String(100), default='TOPCON')
    panel_brand = db.Column(db.String(100))
    panel_model = db.Column(db.String(255))
    panel_watt = db.Column(db.Numeric(10, 2))
    inverter_brand = db.Column(db.String(100))
    inverter_model = db.Column(db.String(255))
    coordinator_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    coordinator_name = db.Column(db.String(255))
    material_cost = db.Column(db.Numeric(14, 2), default=0)
    installation_charges = db.Column(db.Numeric(14, 2), default=0)
    ...
    total_investment = db.Column(db.Numeric(14, 2), default=0)
    annual_generation = db.Column(db.Numeric(14, 2), default=0)
    annual_savings = db.Column(db.Numeric(14, 2), default=0)
    lifetime_savings = db.Column(db.Numeric(14, 2), default=0)
    estimated_savings = db.Column(db.String(100))
    break_even_year = db.Column(db.String(50))
    ...
    coordinator_user = db.relationship('User', foreign_keys=[coordinator_id])
    creator = db.relationship('User', foreign_keys=[created_by])
    items = db.relationship('QuotationItem', backref='quotation', cascade='all, delete-orphan', lazy=True,
                            order_by='QuotationItem.sl_no')
...
class QuotationItem(db.Model):
    ...
    warranty = db.Column(db.String(255))
    unit_price = db.Column(db.Numeric(14, 2), default=0)
    total_price = db.Column(db.Numeric(14, 2), default=0)


def _q_dec(value, default='0'):
    try:
...
def _q_next_number():
    prefix = f"QTN-{date.today().year}-"
    latest = Quotation.query.filter(Quotation.quotation_no.like(prefix + '%')).order_by(Quotation.id.desc()).first()
    n = 1 if not latest else int(latest.quotation_no.rsplit('-', 1)[-1]) + 1
    return f'{prefix}{n:04d}'


def _q_qty_value(qty):
    m = re.search(r'[-+]?\d+(?:\.\d+)?', str(qty or ''))
    return Decimal(m.group()) if m else Decimal('1')


def _q_seed_3kw():
    """Seeds only data supported by the supplied PDF. Other capacities are intentionally configurable."""
    if QuotationConfiguration.query.filter_by(system_type='On Grid', capacity_kw=3).first():
        return
    c = QuotationConfiguration(system_type='On Grid', capacity_kw=3, name='3 kW On Grid')
    db.session.add(c)
    db.session.flush()
    rows = [
        ('Solar PV Modules','UTL','','580 W TOPCON (Glass-to-Glass, DCR)','6 Nos','12 Years Product + 15 Years Service Warranty',6,0),
        ('Solar Inverter','UTL','GT-3-GXT2','3 kW','1 No','10 Years On-Site Replacement Warranty',1,0),
        ('AC Districution Box (ACDB)','Havells','','3 kW','1 No','5 Years',1,0),
        ('DC Distribution Box (DCDB)','Havells','','3 kW','1 No','5 Years',1,0),
        ('DC Cables','Havells / Finolex','','4 Sq.mm','1 Set','Standard Warranty',1,0),
        ('AC Cables','Havells / Finolex','','4 Sq.mm','1 Set','Standard Warranty',1,0),
        ('Energy Meter','HPL','','Single Phase','1 No','5 Years',1,0),
        ('Module Mounting Structure','Apollo','','16 Gauge GI Structure','As Required','Standard Warranty',1,0),
        ('Electrical Accessories','Konseal / Havells','','Pipes, Fittings, Connectors, etc.','1 Set','Standard Warranty',1,0),
        ('Surge Protection Device (SPD)','Havells','','Type 2 SPD','1 Set','Standard Warranty',1,0),
        ('Lightning Arrester','Power On+','','Multi-spike with Down Conductor (50 Sq.mm, 25X3 GI Strip)','1 No','Standard Warranty',1,0),
        ('Earthing System','Power On+','','Chemical Earthing with Charcoal, Salt & Composed','3 Nos','Standard Warranty',3,0),
    ]
    for i, r in enumerate(rows, 1):
        db.session.add(QuotationConfigurationItem(
            configuration_id=c.id, sort_order=i, component=r[0], brand=r[1], model=r[2],
            specification=r[3], qty=r[4], warranty=r[5], qty_value=r[6], unit_price=r[7]
        ))
    db.session.commit()


def _q_fit_text(page, rect, text, fontsize=11, bold=False, align=0, color=(0,0,0)):
    ...
    while size >= 5:
        spare = page.insert_textbox(rect, str(text), fontname='hebo' if bold else 'helv',
                                    fontsize=size, color=color, align=align, overlay=True)
        if spare >= 0:
            return
        # Redact the failed insertion is not reliable; we only reach here on overflow before a visible result.
        size -= 0.5
    page.insert_textbox(rect, str(text)[:120], fontname='helv', fontsize=5, color=color, align=align, overlay=True)


def _q_redact_text(page, text):
    if not text:
        return None
    rects = page.search_for(text)
    if not rects:
        return None
    # Merge rectangles so fragmented currency/text is covered as one field.
    u = rects[0]
    for r in rects[1:]:
        u |= r
...
def _q_redact_all(page, texts):
    found = []
    for t in texts:
        r = _q_redact_text(page, t)
        if r:
            found.append(r)
    if found:
...
def _q_generate_pdf(q, output_path):
    ...
    for page in doc:
        _q_redact_all(page, [
            'Customer name','CUSTOMER NAME','10179','11 April 2026','ON GRID','TOPCON',
            '3  kW','3 kW','UTL','Mr. Vibin C PANICKER','₹ 2,25,000','4,380 UNITS',
            '₹ 35,040','₹ 9,46,080','85% approx.'
        ])

    # Page 3: customer, reference and date.
    p = doc[2]
    _q_fit_text(p, (295,524,455,552), q.customer_name, 11, True)
    _q_fit_text(p, (24,590,100,618), q.reference_no or q.quotation_no, 10, True)
    _q_fit_text(p, (331,590,500,618), q.quotation_date.strftime('%d %B %Y'), 10, True)

    # Page 4: proposed system.
    p = doc[3]
    _q_fit_text(p, (225,312,335,340), q.system_type.upper(), 11, True, 1)
    _q_fit_text(p, (550,326,690,355), q.panel_technology or 'TOPCON', 11, True, 1)
    cap = f'{q.capacity_kw:g} kW'
    _q_fit_text(p, (235,532,330,562), cap, 11, True, 1)
    _q_fit_text(p, (590,532,655,562), q.panel_brand or '', 11, True, 1)
    _q_fit_text(p, (235,740,295,770), cap, 11, True, 1)
    _q_fit_text(p, (560,725,650,755), q.inverter_brand or '', 11, True, 1)

    # Page 6: pricing and savings cards.
    p = doc[5]
    _q_fit_text(p, (480,326,755,386), _q_money(q.total_investment), 26, True, 1, (1,1,1))
    _q_fit_text(p, (675,500,748,530), cap, 11, True, 1)
    _q_fit_text(p, (665,575,790,605), q.estimated_savings or '', 11, True, 1)
    _q_fit_text(p, (695,646,730,675), q.break_even_year or '', 11, True, 1)
    _q_fit_text(p, (30,1020,200,1062), f'{float(q.annual_generation or 0):,.0f} UNITS', 16, True, 1)
    _q_fit_text(p, (245,1020,395,1062), _q_money(q.total_investment), 16, True, 1)
    _q_fit_text(p, (455,1020,590,1062), _q_money(q.annual_savings), 16, True, 1)
    _q_fit_text(p, (650,1020,805,1062), _q_money(q.lifetime_savings), 16, True, 1)
    ...
    cols = [44, 137, 272, 369, 527, 638, 776]
    y = 201
    row_h = 71
    for idx, item in enumerate(q.items, 1):
        if y + row_h > 1080:
            break
    ...
    # Page 8: coordinator and client name/address.
    p = doc[7]
    _q_fit_text(p, (30,725,285,758), f'MR. {q.coordinator_name.upper()}' if q.coordinator_name else '', 11, True)
    _q_fit_text(p, (30,855,300,885), q.customer_name.upper(), 11, True)
    if q.customer_address:
        _q_fit_text(p, (30,886,500,920), q.customer_address, 8.5, False)
...
def quotation_create():
    ...
    customers = Customer.query.order_by(Customer.name).all()
    coordinators = User.query.filter(User.role == 'coordinator', User.is_deleted == False, User.is_active == True).order_by(User.full_name).all()

    if request.method == 'POST':
        customer_id = request.form.get('customer_id') or None
        customer = db.session.get(Customer, int(customer_id)) if customer_id else None
        customer_name = (request.form.get('customer_name') or (customer.name if customer else '')).strip()
        if not customer_name:
            flash('Customer name is required.', 'danger')
            return redirect(url_for('quotation_create'))

        capacity = _q_dec(request.form.get('capacity_kw'))
        system_type = request.form.get('system_type','On Grid')
        cfg = QuotationConfiguration.query.filter_by(system_type=system_type, capacity_kw=capacity, active=True).first()
        if not cfg:
            flash('No quotation configuration exists for this system type and capacity. Add it under Quotation Settings.', 'danger')
            return redirect(url_for('quotation_create'))

        coordinator_id = request.form.get('coordinator_id') or None
        coord = db.session.get(User, int(coordinator_id)) if coordinator_id else None
        coordinator_name = coord.full_name if coord else (request.form.get('coordinator_name') or '')

        material = _q_dec(request.form.get('material_cost'))
        installation = _q_dec(request.form.get('installation_charges'))
        other = _q_dec(request.form.get('other_charges'))
        discount = _q_dec(request.form.get('discount'))
        gst_percent = _q_dec(request.form.get('gst_percent'))
        taxable = max(material + installation + other - discount, Decimal('0'))
        gst_amount = taxable * gst_percent / Decimal('100')
        total = taxable + gst_amount

        q = Quotation(
            quotation_no=_q_next_number(), reference_no=request.form.get('reference_no') or '', quotation_date=date.today(),
            customer_id=customer.id if customer else None, customer_name=customer_name,
            customer_address=(request.form.get('customer_address') or (customer.full_address if customer else '') or ''),
            customer_phone=request.form.get('customer_phone') or (customer.phone if customer else ''),
            customer_email=request.form.get('customer_email') or (customer.email if customer else ''),
            system_type=system_type, capacity_kw=capacity,
            panel_technology=request.form.get('panel_technology') or 'TOPCON',
            panel_brand=request.form.get('panel_brand') or '', panel_model=request.form.get('panel_model') or '',
            panel_watt=_q_dec(request.form.get('panel_watt')),
            inverter_brand=request.form.get('inverter_brand') or '', inverter_model=request.form.get('inverter_model') or '',
            coordinator_id=coord.id if coord else None, coordinator_name=coordinator_name,
            material_cost=material, installation_charges=installation, other_charges=other,
            discount=discount, gst_percent=gst_percent, gst_amount=gst_amount, total_investment=total,
            annual_generation=_q_dec(request.form.get('annual_generation')),
            annual_savings=_q_dec(request.form.get('annual_savings')),
            lifetime_savings=_q_dec(request.form.get('lifetime_savings')),
            estimated_savings=request.form.get('estimated_savings') or '', break_even_year=request.form.get('break_even_year') or '',
            created_by=current_user.id, status='Draft'
        )
        db.session.add(q)
        db.session.flush()

        # Snapshot the configuration so future configuration edits do not change old quotations.
        for idx, item in enumerate(cfg.items, 1):
            make = request.form.get(f'item_make_{item.id}', item.brand or '')
            spec = request.form.get(f'item_spec_{item.id}', item.specification or '')
            qty = request.form.get(f'item_qty_{item.id}', item.qty or '')
            price = _q_dec(request.form.get(f'item_price_{item.id}', item.unit_price or 0))
            q.items.append(QuotationItem(
                sl_no=idx, component=item.component, make=make, model=item.model or '', specification=spec,
                qty=qty, warranty=item.warranty or '', unit_price=price,
                total_price=price * _q_qty_value(qty)
            ))
        db.session.commit()
        return redirect(url_for('quotation_preview', qid=q.id))

    return render_template('quotations/create.html', configs=configs, customers=customers, coordinators=coordinators)
...
def quotation_config_items(config_id):
    ...
    if not cfg:
        return jsonify({'ok':False,'message':'Configuration not found'}), 404
    return jsonify({'ok':True, 'config': {
        'id':cfg.id,'system_type':cfg.system_type,'capacity_kw':float(cfg.capacity_kw),
        'name':cfg.name,'items':[{
            'id':i.id,'component':i.component,'brand':i.brand or '', 'model':i.model or '',
            'specification':i.specification or '', 'qty':i.qty or '', 'qty_value':float(i.qty_value or 0),
            'warranty':i.warranty or '', 'unit_price':float(i.unit_price or 0)
        } for i in cfg.items]
    }})
...
def quotation_preview(qid):
    ...
    output = os.path.join(QUOTATION_OUTPUT_DIR, f'{q.quotation_no}.pdf')
    try:
        _q_generate_pdf(q, output)
    except Exception as exc:
        app.logger.exception('Quotation PDF generation failed')
        flash(f'PDF generation failed: {exc}', 'danger')
        return redirect(url_for('quotations'))
    q.pdf_path = output
    q.status = 'Generated'
    db.session.commit()
    return render_template('quotations/preview.html', quotation=q)
...
def quotation_pdf(qid):
    ...
    if not q.pdf_path or not os.path.isfile(q.pdf_path):
        output = os.path.join(QUOTATION_OUTPUT_DIR, f'{q.quotation_no}.pdf')
        _q_generate_pdf(q, output)
        q.pdf_path = output
        db.session.commit()
    return send_file(q.pdf_path, mimetype='application/pdf', as_attachment=False, download_name=f'{q.quotation_no}.pdf')
...
def quotation_download(qid):
    ...
    if not q.pdf_path or not os.path.isfile(q.pdf_path):
        ...
        q.pdf_path = output
        db.session.commit()
    return send_file(q.pdf_path, mimetype='application/pdf', as_attachment=True, download_name=f'{q.quotation_no}.pdf')
...
def quotation_settings():
    if request.method == 'POST':
        ...
        if action == 'create_config':
            cap = _q_dec(request.form.get('capacity_kw'))
            system_type = request.form.get('system_type','On Grid')
            name = request.form.get('name') or f'{cap:g} kW {system_type}'
            if not QuotationConfiguration.query.filter_by(system_type=system_type, capacity_kw=cap).first():
                db.session.add(QuotationConfiguration(system_type=system_type, capacity_kw=cap, name=name))
                db.session.commit()
                flash('Quotation capacity configuration created.', 'success')
        elif action == 'add_item':
            cfg = db.session.get(QuotationConfiguration, int(request.form.get('config_id')))
            if cfg:
                last = QuotationConfigurationItem.query.filter_by(configuration_id=cfg.id).order_by(QuotationConfigurationItem.sort_order.desc()).first()
                db.session.add(QuotationConfigurationItem(
                    configuration_id=cfg.id, sort_order=(last.sort_order + 1 if last else 1),
                    component=request.form.get('component') or 'New Component', brand=request.form.get('brand') or '',
                    model=request.form.get('model') or '', specification=request.form.get('specification') or '',
                    qty=request.form.get('qty') or '1 No', qty_value=_q_dec(request.form.get('qty_value'), '1'),
                    warranty=request.form.get('warranty') or '', unit_price=_q_dec(request.form.get('unit_price'))
                ))
                db.session.commit()
                flash('Quotation item added.', 'success')
        elif action == 'delete_item':
            item = db.session.get(QuotationConfigurationItem, int(request.form.get('item_id')))
            if item:
                db.session.delete(item); db.session.commit(); flash('Quotation item deleted.', 'success')
        return redirect(url_for('quotation_settings'))

    configs = QuotationConfiguration.query.order_by(QuotationConfiguration.system_type, QuotationConfiguration.capacity_kw).all()
    return render_template('quotations/settings.html', configs=configs)
...
def quotation_template_upload():
    if request.method == 'POST':
        ...
        if not f or not f.filename.lower().endswith('.pdf'):
            flash('Please upload a PDF template.', 'danger')
            return redirect(url_for('quotation_template_upload'))
        f.save(QUOTATION_TEMPLATE_PATH)
        flash('Quotation PDF template updated. The current field coordinates are designed for the supplied template layout.', 'success')
        return redirect(url_for('quotation_template_upload'))
    return render_template('quotations/template.html', template_path=QUOTATION_TEMPLATE_PATH)
...
# Copy the bundled template on first run if no template exists.
_BUNDLED_QUOTATION_TEMPLATE = os.path.join(app.root_path, 'quotation_template.pdf')
if os.path.isfile(_BUNDLED_QUOTATION_TEMPLATE) and not os.path.isfile(QUOTATION_TEMPLATE_PATH):
    try:
        import shutil as _quotation_shutil
        _quotation_shutil.copy2(_BUNDLED_QUOTATION_TEMPLATE, QUOTATION_TEMPLATE_PATH)
    except Exception:
        pass
...
if __name__ == '__main__':
    with app.app_context():
        ...
        try:
            ...
            app.logger.warning('Quotation seed skipped: %s', exc)
        # seed_db()
